    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between px-1">
         <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
            {points.length === 0 ? 'Click map to start' : `${points.length} Boundary Points Selected`}
         </p>
         {points.length >= 3 && (
            <div className="flex items-center gap-1.5 text-success-600 animate-pulse">
               <CheckCircle size={12} />
      ...
      <div className="relative h-[400px] w-full rounded-2xl border border-slate-200 overflow-hidden shadow-inner bg-slate-100 group">
        <MapContainer 
        ...
        >
          ...
          {points.length > 0 && points.map((p, i) => (
            <Marker key={i} position={p} icon={new L.DivIcon({
              ...
              html: `<div class="w-4 h-4 bg-primary-600 rounded-full border-2 border-white shadow-lg flex items-center justify-center text-[8px] text-white font-bold">${i + 1}</div>`,
              iconSize: [16, 16],
              iconAnchor: [8, 8]
            })} />
          ))}
        ...
        <div className="absolute bottom-4 left-4 right-4 z-[1000] flex justify-center pointer-events-none">
           <div className="bg-slate-900/90 backdrop-blur-md text-white px-5 py-2.5 rounded-2xl text-[10px] font-bold uppercase tracking-widest flex items-center gap-3 border border-white/10 shadow-2xl">
      ...
      <div className="flex gap-3">
        <button
          ...
          className="flex-1 btn btn-primary py-4 text-xs uppercase tracking-widest font-bold shadow-lg shadow-primary-200 disabled:opacity-50 disabled:grayscale transition-all active:scale-[0.98]"
        >
          <CheckCircle size={18} />
